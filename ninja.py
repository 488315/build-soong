...
class NinjaRunner:
    ...
    def _build_env(self):
        allowed = {
            "HOME", "LANG", "LC_MESSAGES", "PATH", "PWD", "TMPDIR", "USER",
                ...
                try:
                    if self.log_path.exists():
                        ...
                        if prev_mtime is not None and mtime == prev_mtime:
                            ...
                            print(f"[WARN] [{timestamp}] Ninja build may be stuck (log unchanged).")
                            subprocess.run(["pstree", "-palT", str(os.getpid())])
                        prev_mtime = mtime
                    else:
                        print(f"[DEBUG] Ninja log file '{self.log_path}' does not exist yet.")
    ...
    def run(self, ninja_args):
        ...
        args = [
            ...
            "-j", str(self.config.parallel),
            "-f", self.config.combined_ninja_file,
        ] + ninja_args
